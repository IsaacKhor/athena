...
class AssgnForm(ModelForm):
    ...
    class Meta:
        model = Assignment
        fields = ['title', 'code', 'desc', 'due_date', 'max_grade', 'weight', 'max_subs']
...
class SubmitForm(Form):
    ...
    student = ChoiceField()#This is temporary for testing - should be based on login
    ...
    def __init__(self, assignment, *args, **kw):
        ...
        self.fields['student'].choices = map(lambda s: (s.id, "%s %s" % (s.first_name, s.last_name)), assignment.course.students.all())
...
class GradeForm(ModelForm):
    ...
    def __init__(self, sub, *args, **kw):
        ...
        self.fields['grader'].choices = map(lambda u: (u.id, "%s %s" % (u.first_name, u.last_name)), sub.assignment.course.instructors.all())
